...
Future<File?> showModalBottomSheetImage(BuildContext context)async{
  return await showModalBottomSheet<File>(context: context, builder: (context){
      return Container(
        height: 100,
            ...
            Row(
              ...
              children: <Widget>[
                TextButton.icon(
                  ...
                  label:  Text("Máy ảnh", style: TextStyle(color: mainColor, fontSize: 16),),
                ),
                TextButton.icon(
                  onPressed: () async {
                    if (await requestPhotosPermission()) {
                      await pickImage(ImageSource.gallery).then((value) => Navigator.pop(context,value));
                    }
                  },
                  icon:  Icon(Icons.image,color: mainColor,),
                  label: Text("Thư viện", style: TextStyle(color: mainColor, fontSize: 16),),
                ),
              ],
            ),
          ],
