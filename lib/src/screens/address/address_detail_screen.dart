              ...
              (widget.status == "edit")
                ...
                child: ElevatedButton(
                  ...
                  child: Text("Xóa địa chỉ",
                      style: btnTextStyle.copyWith(color: mainColor)),
                ),
              )
                  : Container(),
              const SizedBox(
                  ...
                  onPressed: () async {
                    ...
                    if (check) {
                      if (widget.status == "edit") {
                        ...
                        await addressProvider.updateAddress(token: user.token!, onSuccess: (AddressModel address){
                          context
                              .read<UserProvider>()
                              .setAddress(address);
                        ...
                        await addressProvider.createAddress(userID: user.userID!, token: user.token!, onSuccess: (List<AddressModel> listAddress){
                          context.read<UserProvider>().setListAddress(listAddress);
                          LoadingDialog.hideLoadingDialog(context);
                          Navigator.pop(context);
                        }, onFailed: (String msg){
                          LoadingDialog.hideLoadingDialog(context);
